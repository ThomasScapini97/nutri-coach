...
export default function Exercise() {
  ...
  const handleDelete = async (exercise) => {
    ...
    if (todayLog?.id) {
      await supabase.from("food_logs").update({ total_burned_calories: newTotal }).eq("id", todayLog.id);
    }
    queryClient.invalidateQueries({ queryKey: ["exercises", TODAY] });
    queryClient.invalidateQueries({ queryKey: ["exercises-week"] });
    queryClient.invalidateQueries({ queryKey: ["foodlog"] });
    ...
    <div className="flex-1 overflow-y-auto pb-24" style={{ background: "#f0fcf3" }}>
      <div style={{ maxWidth: "480px", margin: "0 auto", padding: "20px 16px", display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative" }}>
            <div>
              ...
              <p style={{ fontSize: "42px", fontWeight: 500, lineHeight: 1, letterSpacing: "-1px" }}>
                {totalBurnedToday.toLocaleString()}
              </p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <p style={{ fontSize: "13px", fontWeight: 500, color: "#1a3a22", marginBottom: "8px", padding: "0 2px", display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          <div style={{ background: "white", borderRadius: "16px", padding: "14px", border: "0.5px solid rgba(0,0,0,0.06)" }}>
            <div style={{ display: "flex", alignItems: "flex-end", gap: "6px", height: "80px" }}>
              {weekDays.map((day, i) => {
                ...
                const barHeight = day.burned > 0 ? Math.max(8, (day.burned / maxBurned) * 56) : 4;
                return (
                  <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "4px", height: "100%" }}>
                    ...
                    {day.burned > 0 && (
                      <span style={{ fontSize: "8px", color: "#9ca3af" }}>{day.burned}</span>
                    )}
                  </div>
                );
              })}
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  width: "100%", background: selectedExercise && minutes ? "#dc2626" : "#f3f4f6",
                  color: selectedExercise && minutes ? "white" : "#9ca3af",
                  border: "none", borderRadius: "14px", padding: "14px",
                  fontSize: "14px", fontWeight: 500, cursor: selectedExercise && minutes ? "pointer" : "default",
                  fontFamily: "inherit",
                }}
              >
